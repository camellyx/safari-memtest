...
void print_failing_info() {
  int j, err_idx, mag;
  ulong h;

  for (err_idx=0; err_idx<v->err_range.size; err_idx++) {
    pop2clear();
    cprint(POP2_Y+1, POP2_X+2, "Consecutive Failing Addresses:");
    cprint(POP2_Y+3, POP2_X+14, " of ");
    dprint(POP2_Y+3, POP2_X+19, v->err_range.size,4,0);
    cprint(POP2_Y+5, POP2_X+6, "Errors in range: ");
    cprint(POP2_Y+6, POP2_X+6, "Range Stride: ");
    cprint(POP2_Y+7, POP2_X+6, "High Address: 0x");
    cprint(POP2_Y+8, POP2_X+6, "Low Address:  0x");
    cprint(POP2_Y+9, POP2_X+6, "Histogram of faulty bits: <-bit 31, bit 0 ->");
    cprint(POP2_Y+13, POP2_X+6, "Hit 1 to Continue, 0 to quit"); //option 1
    //cprint(POP2_Y+13, POP2_X+6, "Hit a key to Continue"); //option 2

    //loop through all records
    dprint(POP2_Y+3, POP2_X+6, err_idx+1,8,0);//number
    hprint(POP2_Y+5, POP2_X+22, v->err_range.ranges[err_idx].count_per_range);//err count per range
    hprint(POP2_Y+6, POP2_X+22, v->err_range.ranges[err_idx].range_stride);//stride per range
    hprint(POP2_Y+7, POP2_X+22, v->err_range.ranges[err_idx].high_addr);//high addr
    hprint(POP2_Y+8, POP2_X+22, v->err_range.ranges[err_idx].low_addr);//low addr
    //print histogram
    for (j=0;j<32;j++){
      mag = 0;
      ...
      dprint(POP2_Y+10, POP2_X+6+(j*5), mag,4,0);
    }
    //i = getval(POP2_Y+12, POP2_X+34, 0); //option 1
    //wait_keyup(); //option 2
